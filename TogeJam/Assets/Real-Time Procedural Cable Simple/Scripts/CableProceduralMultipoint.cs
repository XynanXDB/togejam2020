...
public class CableProceduralMultipoint : MonoBehaviour
{
	...
	[SerializeField, Tooltip("Series of points in cable.  If this transform is the desired start, add it to this array.")] protected CableSection[] cableSections;

	// Number of points per meter
	[SerializeField, Tooltip("Number of points per unit length, using the straight line from the start to the end transform.")] protected float pointDensity = 3;
	// These are used later for calculations
	Vector3 sagDirection;

	// cable section
	[Serializable] protected class CableSection
	{
